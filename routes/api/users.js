...
router.post('/users/changepassword', auth.required, function (req, res, next) {
  ...
  User.findById(req.payload.id).then(function (user) {
    ...
    if (!req.body.user.currentPassword) {
      return res.status(422).json({ errors: { currentPassword: "can't be blank" } });
    }
    ...
    User.findOne(
      { username: req.body.user.username })
      .then(function (user) {
        var status = user.validPassword(req.body.user.currentPassword);
        if (!status) {
...
router.post('/users/resetAccount', function (req, res, next) {
  User.findOne(
    { username: req.body.user.username })
    .then(function (user) {

      if (user == null) {
        return res.status(422).json({ errors: { username: "User Does not Exist" } });
      }
      var password = generatePassword(8, false);
      user.setPassword(password);
      user.save().then(function () {
        mailsender.sendAccountReset(user.username, user.username, password)
          .then(sendStatus => {
            return res.json({ status: "SUCCESS" });
          })
          .catch(err => {
